...
namespace Csharp
{
    public class Solution
    {
        ...
        public long partTwo()
        {
            ...
            for(int i = 0; i < lines.Length; i++){
                storage[i] = (long) Int32.Parse(lines[i]);
                storage[i] *= 811589153;
            }
            //Move in order
            int[] indices = (int[]) Enumerable.Range(0, lines.Length).ToArray();
            ...
            for(int iter = 0; iter < 1; iter++){
                for(int i = 0; i < lines.Length; i++){
                    int index = Array.IndexOf(indices, i);
                    int swapIndex = (int) ((long)index + (storage[index] % lines.Length))%lines.Length;
                    if(swapIndex < 0){
                        swapIndex+=lines.Length-1;
                    }
                    if(swapIndex == 0){
                        swapIndex = lines.Length-1;
                    }
                    else if(swapIndex == lines.Length-1){
                        swapIndex = 0;
                    }
                    Console.WriteLine();
                    ...
                    if(swapIndex < index){
                        ...
                        swapArr = newArr[index];
                        indSwap = indices[index];
                        for(int u = index; u > (index - swapIndex); u--){
                            newArr[u] = newArr[u-1];
                            indices[u] = indices[u-1];
                        }
                        newArr[index - swapIndex] = swapArr;
                        indices[index - swapIndex] = indSwap;
                    }
                    //Going forward
                    else{
                        swapArr = newArr[index];
                        indSwap = indices[index];
                        for(int u = index; u<swapIndex; u++){
                            newArr[u] = newArr[u+1];
                            indices[u] = indices[u+1];
                        }
                        newArr[swapIndex] = swapArr;
                        indices[swapIndex] = indSwap;
                    }
