    ...
    public class Solution
    {
        ...
        public int partTwo()
        {
            ...
            for(int y = 0; y < max; y++){
                ...
                List<Int32> clear = Enumerable.Range(0, max+1).ToList();
                for(int i = 0; i < numSensors; i++){
                    if(!(range[i, 0] == 0 && range[i, 1] == 0)){
                        int stindex = clear.IndexOf(range[i, 0]);
                        int findex = clear.IndexOf(range[i, 1]);
                        clear.RemoveRange(stindex, findex-stindex+1);
                    }
                }
                int x = 0;
                if(clear.Count != 0){
